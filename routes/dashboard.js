...
router.get('/', auth, (req, res) => {
    ...
    /* This route is protected by the auth middleware, which will redirect users to the login screen if they are not 
    logged in. It is then determined whether the user is a moderator, an admin, or a regular verified user. If the user 
    is a moderator or an admin, there are extra options for them to choose from on the dashboard page. */
...
router.get('/reports', (req, res) => {
    try {
        ...
        /* Serves the report queue if the user is an administrator or a moderator, and redirects them to the dashboard 
        if not. */
...
router.post('/settings/pick', auth, async (req, res) => {
    try {
        ...
        /* This endpoint recieves a request by users to change the badge that appears next to their names when they 
        post images and comments. For each case, the user's session is analyzed and if they have the proper credentials 
        to be using the requested badge, then their request is granted. Admins can have the admin, moderator or 
        verified badge, moderators can have the moderator or verified badge, and verified users can only use the 
        verified badge. All users may post anonymously if they'd like. */
