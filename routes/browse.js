// This one is very long, I might split it up into separate routes in the future. It is responsible for serving all of the possible pages on the browse tab. This includes all the possible configurations one can choose in the "Sort by" options. All routes have the "nsfw" middleware. 
...
router.get('/', nsfw, async (req, res) => {
    try {
        ...
        // Serves the Browse page with the 49 most recent images. I do not have to sort by date because image ids are chronological.
        ...
        // Creates an array of objects containing information about each image which will be looped through when the page is rendered using Handlebars. 
...
router.get('/recent-comments', nsfw, async (req, res) => {
    try {
        ...
        // Every image has a column called "latest_comment". It is null for images without comments, but every time a comment is posted on an image, this column is updated with the post number of the comment. Since comments are also chronological, I can sort using this field
...
router.get('/recent-comments/:id', nsfw, async (req, res) => {
    try {
        ...
        // The :id parameter here is the page number. This id is checked to make sure it is actually a number. If it is not, the user will be redirected to the 404 page. If it is a number, we calculate the image offset using the formula (:id - 1) * 49. 
...
router.get('/popular/year', nsfw, async (req, res) => {
    try {
        ...
        // Determines the current year, then finds the 49 most commented on images from this year (so, not necessarily from the last 365 days)
...
router.get('/popular/year/:off/:id', nsfw, async (req, res) => {
    try {
        ...
        // The :off parameter is the number of years we are going back, while the :id parameter is the page number from the images in this year. Both parameters are determined to be numbers, then the desired year is determined, the desired offset is determined, and the images are pulled from the database and sorted by number of comments.
...
router.get('/popular/month/:off/:id', nsfw, async (req, res) => {
    try {
        ...
        // :off is used to determine how many months back the user would like to go. It does this by determining the current month and year, then subtracting the offset from the month. While the month is less than one, the current year is subtracted by one and 12 is added to the month. Once we have a valid month and year, images matching these are grabbed from the database offset by the :off parameter. They are sorted by the number of comments.
...
router.get('/popular/day/:off/:id', nsfw, async (req, res) => {
    try {
        ...
        // Determines the desired day by using the built-in setDate function and simply subtracting the days. The :id of course is the page number
